...
class ChartDownNanScreen extends Component{
    constructor(props){
        super(props);
    }
    ...
    onClickSave(val){
        if(val.length > 0){
            var list_bookmark = [];
            try {
                getAsyncStorage((this.props.select_hour == 0) ? "BOOK_MARK_DOWN_NAN" : "BOOK_MARK_DOWN_NAN_24H").then((list_data) => {
                    getDateTime().then((datetime) => {
                        if(list_data != null) {
                            list_bookmark = list_data.reverse();
                        }
                        list_bookmark.push({id: getIdByDateTime(), datetime: datetime, data: val});
                        setAsyncStorage((this.props.select_hour == 0) ? "BOOK_MARK_DOWN_NAN" : "BOOK_MARK_DOWN_NAN_24H", JSON.stringify(list_bookmark.reverse()));
                        this.showAlert();
                    })
                })
            } catch (error) {}
        ...
        return(
            <View style={styles.contain}>
                <View style={styles.contain_view_header_list}>
                    <Text style={styles.contain_view_header_list_text}>{this.props.list_calculate_down_nan.length}</Text>
                </View>
                <FlatList
                    style={styles.contain_list}
                    data = {this.props.list_calculate_down_nan}
                    renderItem={({item}) => (
                        this.renderItemList(item)
                    )}
                    keyExtractor={item => item.symbol}
                />
                { this.props.list_calculate_down_nan.length > 0 && <TouchableOpacity 
                    style={styles.contain_touch}
                    onPress={()=>{this.onClickSave(this.props.list_calculate_down_nan);}}
                >
                    <Image
...
function mapStateToProps(state){
    return{
        list_calculate_down_nan: state.KC.list_calculate_down_nan,
...
export default connect(mapStateToProps, mapDispatchToProps)(ChartDownNanScreen);